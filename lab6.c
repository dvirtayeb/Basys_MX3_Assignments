...
void setupTimers() {
    //Timer 4
    PR4 = 0x7A11; //             set period register, generates one interrupt every 1 ms
    ...
    //Timer 1
    PR1 = 0x1869; //             set period register, generates one interrupt every 1 ms
...
void writeSecondRow(char ch) {
    jumpLCDTo(SECOND_ROW_RIGHT_BOUNDARY - FIRST_ROW_LEFT_BOUNDARY - 2);
    PORTE = calculateChar(ch / 10);
    runLCDAction();
    PORTE = calculateChar(ch % 10);
    runLCDAction();
}
...
void __ISR(_TIMER_3_VECTOR, ipl6) Timer23SR(void) {
    PORTDbits.RD12 = 1- PORTDbits.RD12;
    IFS0bits.T3IF = 0;
}
